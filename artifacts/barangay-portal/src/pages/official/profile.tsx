        ...
        <Card className="p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              ...
              { icon: <User className="w-4 h-4" />, label: "Position", value: officialInfo?.position ?? "Barangay Official" },
              { icon: <Mail className="w-4 h-4" />, label: "Official Email", value: userData?.email },
              { icon: <Phone className="w-4 h-4" />, label: "Phone", value: userData?.phone ?? "0912-345-6789" },
            ].map((item, i) => (
            ...
            {mockOfficials.map(o => (
              <div key={o.id} className="flex items-center gap-3 p-3 bg-muted/40 rounded-lg">
                <div className="w-9 h-9 rounded-full bg-primary/10 border-2 border-primary/20 flex items-center justify-center shrink-0">
                  <span className="text-sm font-bold text-primary">{o.name.replace(/^(Hon\.|Dr\.)\s*/i,"").charAt(0)}</span>
                </div>
                <div className="flex-1 min-w-0">
